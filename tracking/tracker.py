...
class Tracker(object):
    """
        Box distance: Euclid distance between centers of 2 boxes
        Assign identity for object: Darknet tracking algorithm (baseline)
        Velocity tracking: alpha-beta filter

    """
    def __init__(self, story_num = 10, video_fps = 25):
        self.buffer = []
        self.max_buffer_size = story_num
        self.max_tracking_id = 0
        self.video_fps = video_fps
        self.tracking_period = 3 #for velocity tracking, unit: frame
        self.meter_per_px = 1 # 9.0 / 720
    ...
    def _update_buffer(self, cur_object_infos):
        if(len(self.buffer) == self.max_buffer_size):
            self.buffer.pop(-1)
        self.buffer.insert(0, cur_object_infos)

    def _estimate_velocity(self, v_prev, z_cur, kalman_gain = 0.2):
        """"
    ...
    def track_object(self, cur_object_infos, max_dist_thres = 30):
        """
        ...
        if(num_cur_boxes == 0):
            self._update_buffer(cur_object_infos)
            return None
        ...
        if not has_previous:
            ...
            tracked_objects = [None] * num_cur_boxes # store the latest object that having same track_id with object in current step
            for i in range(num_cur_boxes):
            ...
            for i in range(num_cur_boxes):
                if(min_dists[i] < max_dist_thres and tracked_objects[i] is not None):
                    cur_object_infos[i].track_id = tracked_objects[i].track_id
                    z_n = self._measure_velocity(min_dists[i], cur_object_infos[i].frame_id - tracked_objects[i].frame_id)
                    ...
                    if (cur_object_infos[i].frame_id % self.tracking_period == 0):
                        ...
                        if(tracked_objects[i].velocity == 0.0): # compare float number (?)
                            cur_object_infos[i].velocity = z_n
                        else:
                            cur_object_infos[i].velocity = self._estimate_velocity(tracked_objects[i].velocity, z_n)
